angular.module('mainApp').controller('signinCtrl',function($scope,$routeParams,signinService,$location,$rootScope){
     $scope.login = function(){
     	signinService.post({email:$scope.email,password:$scope.password},"api/login",function(response,err){
     		if(err){
     			alert(err.success);
     			console.log(err);
     		}
...
angular.module('mainApp').controller('signupCtrl',function($scope,$routeParams,signinService,$location,$rootScope){
     ...
     $scope.changeRoute = function(url, forceReload) {
      ...
      $scope.signUp = function(){
        ...
        signinService.post($scope.user,"api/signup",function(response,err){
            if(err){
...
angular.module('mainApp').controller('companyCtrl',function($scope,$routeParams,signinService,$location,$rootScope){
     ...
     $scope.changeRoute = function(url, forceReload) {
      ...
      $scope.addCompany = function(){
        ...
        signinService.post($scope.company,"api/companies",function(response,err){
            if(err){
                //alert(err.success);
                console.log(err);
            }
                    ...
                    signinService.post($scope.company,"api/updateUserCompany",function(response,err){
                        if(err){
                            //alert(err.success);
                            console.log(err);
                        }
